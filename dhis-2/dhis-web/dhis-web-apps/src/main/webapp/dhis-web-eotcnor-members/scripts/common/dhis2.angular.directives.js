    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            function setSelectedOu(ids, names) {
                ...
                if( ids[0] && names[0] ){
                    var _ou = {id: ids[0], displayName: names[0]};

                    IndexDBService.open('dhis2ou').then(function(){
                        IndexDBService.get('ou', _ou.id).then(function(ou){
                            if( ou ){
                                ou.id = ids[0];
                                ou.displayName = ou.n;
                                $timeout(function () {
                                ...
                                IndexDBService.get('ouPartial', _ou.id).then(function(ou){
                                    if( ou ){
                                        ou.id = ids[0];
                                        ou.displayName = ou.n;
                                        $timeout(function () {
...
.directive('serversidePaginator', function factory() {
    ...
    return {
        ...
        templateUrl: 'angular-forms/serverside-pagination.html'
    };
})
